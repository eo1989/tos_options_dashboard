...
def prob_cone(stock_price: float,
              volatility: float,
              days_ahead: int,
              probability=0.7,
              trading_periods: int = 252) -> tuple:
    ...
    std_dev = z_score * stock_price * volatility * math.sqrt(
        days_ahead / trading_periods)
...
def get_prob(stock_price: float,
             strike_price: float,
             volatility: float,
             days_ahead: int,
    ...
    if None or 0 not in (stock_price, strike_price, volatility, days_ahead):
        ...
        z_score = abs(stock_price - strike_price) / (
            stock_price * volatility * math.sqrt(days_ahead / trading_periods))
...
def get_hist_volatility(price_df,
                        window=30,
                        estimator='log_returns',
    ...
    if estimator == 'log_returns':
        ...
        log_return = (price_df['close'] / price_df['close'].shift(1)).apply(
            np.log)

        result = log_return.rolling(
            window=window, center=False).std() * math.sqrt(trading_periods)
        ...
        rs = (1.0 / (4.0 * math.log(2.0))) * (
            (price_df['high'] / price_df['low']).apply(np.log))**2.0
